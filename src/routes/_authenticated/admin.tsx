...
import { Search, LogOut, Shield, ArrowUpDown } from "lucide-react";
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            ...
            <button onClick={signOut} className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1 hover:bg-slate-100">
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data: brands, error } = await supabase
        .from("brands")
        .select("id, name, plan, subscription_status, billing_cycle, renewal_date, owner_user_id, max_staff_accounts, max_locations, created_at");
      ...
      for (const l of locRes.data ?? []) locCount.set(l.brand_id, (locCount.get(l.brand_id) ?? 0) + 1);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">All brands across the platform. Sorted by renewal date — soonest first.</p>
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[240px] max-w-md">
          <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-400" />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search brand or owner email" className="pl-8 bg-white" />
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {rows.map((b) => {
                ...
                return (
                  <TableRow key={b.id} className={cn("hover:bg-slate-50", flag === "overdue" && "bg-red-50/50", flag === "soon" && "bg-amber-50/40")}>
                    <TableCell>
                      <Link to="/admin/brands/$id" params={{ id: b.id }} className="font-medium text-slate-900 hover:underline">
                        {b.name}
                      </Link>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell><span className="capitalize">{b.plan}</span></TableCell>
                    <TableCell><StatusBadge status={b.subscription_status} /></TableCell>
                    <TableCell className="capitalize text-sm">{b.billing_cycle}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className="text-sm">{b.renewal_date ? format(parseISO(b.renewal_date), "MMM d, yyyy") : "—"}</span>
                        {flag === "overdue" && <Badge className="bg-red-600 hover:bg-red-600">Overdue</Badge>}
                        {flag === "soon" && <Badge className="bg-amber-500 hover:bg-amber-500">Soon</Badge>}
                      </div>
                    </TableCell>
                    <TableCell className="text-right text-sm">{b.locationCount} / {b.max_locations}</TableCell>
                    <TableCell className="text-right text-sm">{b.staffAccounts} / {b.max_staff_accounts}</TableCell>
                    <TableCell className="text-sm text-slate-500">{format(parseISO(b.created_at), "MMM d, yyyy")}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
